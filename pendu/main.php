...
// méthode qui permet d'afficher le tableau passé en paramètre et de séparer les lettres par des espaces
...
// méthode qui cherche toutes les occurrences d'une lettre passée en paramètre dans un tableau de caractères passé aussi en paramètre. Cette méthode retourne toutes les positions dans un tableau
...
{
    $slice = array_slice($tab, $depart);
    $resulta = array_search($lettre, $slice);
    if ($resulta === false) {
        //pour eviter que 0 " === "false
        return [];
    } else {
        $rep[] = $resulta + $depart;
        $tabRes = array_merge($rep, testerLettre($lettre, $tab, $resulta + $depart + 1));
        return $tabRes;
    }
}
...
function ajouterLesLettres($lettre, $tab, $listePosition)
{
    for ($i = 0; $i < count($listePosition); $i++) {
        $tab = ajouterUneLettre($lettre, $tab, $listePosition [$i]);
    }
    return $tab;
}
...
{
    foreach ($listeLettres as $elment) {
        echo ($elment);
    }
}
...
{
    $char = readline("Entrer une lettre :  \n");
...
function testerGagner($nberreur, $tab)
{
    ...
    if (array_search("_", $tab)) {
        if ($nberreur < 9 ){
            //la partie continue;
            return 0;
        } else {
            //la partie s arrete trop d erreur
            return -1;
        }
    } else {
        //partie gagné
...
{   
    $niveau = readline("Choisir un niveau de dificulté: 1 = facile, ou 2 moyen");
    $mot = choisirMot();
    $tabMot = str_split($mot);
    $motCoder = coderMot($mot, $niveau);
    echo($mot);
    ...
    do {
        ...
        //n'affiche pas les mauvaises lettre si le tab est vide
        if (!empty($tabMauvaiseLettre)) {
            afficherMauvaisesLettres($tabMauvaiseLettre);
        }

        $lettre = demanderLettre();
        $tabPos = testerLettre($lettre, $tabMot, 0);

        if (empty($tabPos)) {
            // si la lettre n'est pas dans le mot
            $nbErr++;
            $tabMauvaiseLettre[] = $lettre;
        } else {
           $motCoder =  ajouterLesLettres($lettre, $motCoder, $tabPos);
        }
   ...
   $gagne = testerGagner($nbErr, $tabMot);

    //affiche si gagner ou perdu a la fin
    if ($gagne == 1) {
